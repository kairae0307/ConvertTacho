...
namespace GMap.NET.WindowsForms
{
...
#endif
   {
         ...
         foreach(GMapPolygon r in Polygons)
         {
            ...
            if(r.IsVisible)
            {
               using(GraphicsPath rp = new GraphicsPath())
               {
				  ...
				  else if (r.Name == "polygon1")
				   {
					  
						   for (int i = 60; i < r.LocalPoints.Count; i++)
						   {
				   ...
				   else if (r.Name == "polygon2")
				   {
					   

						   for (int i = 120; i < r.LocalPoints.Count; i++)
						   {
							   GPoint p2 = r.LocalPoints[i];
                ...
                if(rp.PointCount > 0)
                 {
						  //     rp.CloseFigure();
						  //     g.FillPath(r.Fill, rp);   //  그리기
					  if (r.Name == "polygon" )
					  {
						  g.DrawPath(r.Stroke, rp); // 라인 그리기
					  }
					  else if (r.Name == "polygon1")
					  {
						  g.DrawPath(r.Stroke1, rp);
					  }
					  else if (r.Name == "polygon2" )
					  {
						  g.DrawPath(r.Stroke2, rp);
					  }
					  else
					  {
						  g.DrawPath(r.Stroke, rp); // 라인 그리기
      ...
      public void OnDeserialization(object sender)
      {
		 ...
		 foreach (GMapPolygon polygon1 in deserializedPolygonArray)
		 {
			 polygon1.Overlay = this;
			 this.Polygons.Add(polygon1);

		 }
		 foreach (GMapPolygon polygon2 in deserializedPolygonArray)
		 {
			 polygon2.Overlay = this;
